...
public class EntityMonstrousNightmare extends AbstractDragonBase {

    private static final EntityDataAccessor<Integer> DRAGON_VARIANT = SynchedEntityData.defineId(EntityMonstrousNightmare.class, EntityDataSerializers.INT);
    ...
    public EntityMonstrousNightmare(EntityType<? extends TamableAnimal> type, Level level) {
        super(type, level);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if(this.isFlying()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("MonstrousNightmare-Fly", true));
            return PlayState.CONTINUE;
        }
        if(event.isMoving() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("MonstrousNightmare-Walk", true));
            return PlayState.CONTINUE;
        }
        if(this.isInSittingPose() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("MonstrousNightmare-Sit", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("MonstrousNightmare-Idle", true));
        return PlayState.CONTINUE;
    }
