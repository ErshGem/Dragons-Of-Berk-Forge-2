...
public class EntityZippleback extends AbstractDragonBase {

    private static final EntityDataAccessor<Integer> DRAGON_VARIANT = SynchedEntityData.defineId(EntityZippleback.class, EntityDataSerializers.INT);
    ...
    public EntityZippleback(EntityType<? extends TamableAnimal> type, Level level) {
        super(type, level);
    }

    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if(this.isFlying()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("Zippleback-Fly", true));
            return PlayState.CONTINUE;
        }
        if(event.isMoving() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("Zippleback-Walk", true));
            return PlayState.CONTINUE;
        }
        if(this.isInSittingPose() && this.onGround) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("Zippleback-Sit", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("Zippleback-Idle", true));
        return PlayState.CONTINUE;
    }
